...
int main(int argc, char **argv) {
	...
	for (zz=0; zz<BENCH_LOOPS; zz++) {
		...
		element_from_hash(h_enodeb, hash, sha256_info->size);

		element_pow_zn(enodeb_sig, h_enodeb, enodeb_secret_key);
		...
		mbedtls_md(sha256_info, message_cn, sizeof(message_cn), hash);
		element_from_hash(h_cn, hash, 32);

		mbedtls_md(sha256_info, message_mme, sizeof(message_mme), hash);
		element_from_hash(h_mme, hash, 32);

		mbedtls_md(sha256_info	, message_enodeb, sizeof(message_enodeb), hash);
		element_from_hash(h_enodeb, hash, 32);
		...
		element_pairing(temp3, h_enodeb, enodeb_public_key);
		element_mul(temp2, temp2, temp3);
